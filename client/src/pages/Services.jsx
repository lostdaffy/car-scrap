...
const Services = () => {
  const mainServices = [
    {
      ...
      benefits: "₹15,000 - ₹50,000 value + tax benefits"
    },
    {
  ...
  return (
    <>
            ...
            {mainServices.map((service, index) => (
              <div key={index} className="bg-gray-50 rounded-2xl p-8 hover:shadow-lg transition-all duration-300 group">
                <div className="flex items-start gap-6">
                  <div className="w-16 h-16 bg-[#E0A75E] rounded-2xl flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform">
                    <i className={`${service.icon} text-2xl text-white`}></i>
                  </div>
                  
                  <div className="flex-1">
                    <h3 className="text-2xl font-bold text-gray-900 mb-2">{service.title}</h3>
                    <p className="text-[#E0A75E] font-semibold mb-4">{service.subtitle}</p>
                    <p className="text-gray-600 mb-6 leading-relaxed">{service.description}</p>
                    
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
                      {service.features.map((feature, idx) => (
                        <div key={idx} className="flex items-center gap-2">
                          <i className="ri-check-line text-[#E0A75E] flex-shrink-0"></i>
                          <span className="text-gray-700 text-sm">{feature}</span>
                        </div>
                      ))}
                    </div>
                    
                    <div className="bg-[#E0A75E]/10 rounded-lg p-4 border-l-4 border-[#E0A75E]">
                      <div className="flex items-center gap-2">
                        <i className="ri-gift-line text-[#E0A75E]"></i>
                        <span className="font-semibold text-gray-900">{service.benefits}</span>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {specializedServices.map((service, index) => (
              <div key={index} className="bg-white rounded-2xl p-6 text-center shadow-md hover:shadow-lg transition-all duration-300 group">
                <div className="w-16 h-16 bg-[#E0A75E]/10 rounded-2xl flex items-center justify-center mx-auto mb-4 group-hover:bg-[#E0A75E]/20 transition-colors">
                  <i className={`${service.icon} text-2xl text-[#E0A75E]`}></i>
                </div>
            ...
            {processSteps.map((step, index) => (
              <div key={index} className="relative text-center group">
                ...
                {index < processSteps.length - 1 && (
                  <div className="hidden lg:block absolute top-16 left-full w-full h-0.5 bg-[#E0A75E]/30 z-0">
                    <div className="absolute right-0 top-1/2 w-2 h-2 bg-[#E0A75E] rounded-full transform -translate-y-1/2"></div>
                  </div>
                )}
                
                <div className="relative z-10">
                  <div className="w-20 h-20 bg-[#E0A75E] rounded-2xl flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform shadow-lg">
      ...
      {/* Service Guarantee */}
      <section className="py-16 bg-[#E0A75E]/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-white rounded-3xl p-8 shadow-lg">
            ...
            <div className="grid md:grid-cols-3 gap-8">
              <div className="text-center">
                <div className="w-16 h-16 bg-[#E0A75E] rounded-full flex items-center justify-center mx-auto mb-4">
                  <i className="ri-shield-check-line text-2xl text-white"></i>
                </div>
                <h3 className="text-xl font-bold text-gray-900 mb-3">100% Legal Compliance</h3>
                <p className="text-gray-600 text-sm">All processes follow government regulations with proper documentation</p>
              </div>
              ...
              <div className="text-center">
                ...
                <p className="text-gray-600 text-sm">We ensure you get maximum value for your vehicle with transparent pricing</p>
              </div>
              ...
              <div className="text-center">
                ...
                <h3 className="text-xl font-bold text-gray-900 mb-3">24-48 Hour Process</h3>
                <p className="text-gray-600 text-sm">Quick turnaround with same-day pickup and immediate documentation</p>
              </div>
            </div>
          </div>
        </div>
